...
class NetG(nn.Module):
    def __init__(self, ngf=64, nz = 64):
        ...
        self.to1 = nn.Sequential(nn.Conv2d(ngf // 2 , ngf, kernel_size=4, stride=2, padding=1),  # 128
                                 nn.LeakyReLU(0.2, True))
        self.to2 = nn.Sequential(nn.Conv2d(ngf  , ngf * 2, kernel_size=4, stride=2, padding=1),  # 64, add z
                                 nn.LeakyReLU(0.2, True))
        self.to3 = nn.Sequential(nn.Conv2d(ngf * 3 , ngf * 4, kernel_size=4, stride=2, padding=1),  # 32, add hint 
                                 nn.LeakyReLU(0.2, True))
        self.to4 = nn.Sequential(nn.Conv2d(ngf * 4, ngf * 8, kernel_size=4, stride=2, padding=1),  # 16
                                 nn.LeakyReLU(0.2, True))

        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.alterz = nn.Sequential(nn.Conv2d(ngf * 8 + nz, ngf * 8, kernel_size=4, stride=2, padding=1), nn.ReLU(), #16->8
                                    nn.Conv2d(ngf * 8, ngf * 8, kernel_size=3, stride=1, padding=1), nn.ReLU(), 
                                    nn.Conv2d(ngf * 8, ngf * 8, kernel_size=4, stride=2, padding=1), nn.ReLU(), #8->4
                                    nn.Conv2d(ngf * 8, ngf * 8, kernel_size=3, stride=1, padding=1), nn.ReLU(), 
                                    nn.Conv2d(ngf * 8, ngf * 8, kernel_size=4, stride=2, padding=1), nn.ReLU(), #4->2
                                    nn.Conv2d(ngf * 8, ngf * 8, kernel_size=3, stride=1, padding=1), nn.ReLU(),
                                    nn.Conv2d(ngf * 8, ngf * 8, kernel_size=4, stride=2, padding=1), nn.ReLU(), #2->1
                                    nn.Conv2d(ngf*8, nz, kernel_size=3, stride=1, padding=1)
        )
        ...
        self.ske_decoder3 = self.skeleton_decoder(ngf*8, ngf*4) # ->32
        ...
        self.tunnel3 = nn.Sequential(nn.Conv2d(ngf * 8 + ngf*4, ngf * 4, kernel_size=3, stride=1, padding=1),
                                     nn.LeakyReLU(0.2, True),
                                    #  tunnel3,
        )
        ...
        self.ske_decoder2 = self.skeleton_decoder(ngf*4 + ngf*4, ngf*2) # ->64
        ...
        self.tunnel2 = nn.Sequential(nn.Conv2d(ngf * 4 + ngf*2, ngf * 2, kernel_size=3, stride=1, padding=1),
                                     nn.LeakyReLU(0.2, True),
                                    #  tunnel2,
        )
        ...
        self.ske_decoder1 = self.skeleton_decoder(ngf*2 + ngf*2 , ngf) # ->128
        ...
        self.tunnel1 = nn.Sequential(nn.Conv2d(ngf * 2 +ngf, ngf, kernel_size=3, stride=1, padding=1),
                                     nn.LeakyReLU(0.2, True),
        )
                                    #  tunnel1,
        ...
        self.ske_decoder0 = self.skeleton_decoder(ngf + ngf , ngf//2) # ->256
    ...
    def forward(self, sketch, hint, z, skeleton_output=False, has_hint=False, output_z = False, guide_decoder_output=False, sketch_feat=None):
        hint = self.toH(hint)
        x0 = self.to0(sketch)
        x1 = self.to1(x0) 
        x2 = self.to2(x1) 
        ...
        if has_hint:
            ...
            z = self.alterz(torch.cat([x, z_img], 1))
            z = self.avg_pool(z)
        ...
        decoder_output = self.deconv_for_decoder(x)
        ...
        if guide_decoder_output:
            if output_z:
                if skeleton_output:
                    ske = torch.tanh(self.ske_exit(torch.cat([ske, x0], 1)))
                    return x, ske, z, decoder_output
                else:
                    return x, z, decoder_output
            else:
                if skeleton_output:
                    ske = torch.tanh(self.ske_exit(torch.cat([ske, x0], 1)))
                    return x, ske, decoder_output
                else:
...
class NetD(nn.Module):
    def __init__(self, ndf=64):
        ...
        self.feed1 = nn.Sequential(nn.Conv2d(3, ndf, kernel_size=7, stride=1, padding=3, bias=False),  # 256
                                  nn.LeakyReLU(0.2, True),
        ...
        self.feed5 = nn.Sequential(nn.Conv2d(ndf * 4, ndf * 8, kernel_size=3, stride=1, padding=1, bias=False),  # 8
                                   nn.LeakyReLU(0.2, True),
