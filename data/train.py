...
class ImageFolder(data.Dataset):
    def __init__(self, root, transform=None, vtransform=None, stransform=None):
        imgs = make_dataset(root, 'Illustration/256')
    ...
    def __getitem__(self, index):
        ...
        Cimg = color_loader(os.path.join(self.root, 'Illustration/256', fname))

        p = random.random()

        if p < 0.2:
            sketch_folder = 0 #XDOG
            skeleton_folder = 0 
        elif p < 0.2+0.4:
            sketch_folder = 1 #A2S
            skeleton_folder = 1
        else:
            sketch_folder = 2 #simplify
            skeleton_folder = 1

        filename = os.path.splitext(fname)[0]
        Simg = sketch_loader(os.path.join(self.root, 'Sketch', str(sketch_folder), str(256), filename+'.png'))
        Skimg = sketch_loader(os.path.join(self.root, 'Skeleton', str(256), str(skeleton_folder), filename+'.png')) #just the way it is stored

        Cimg, Simg, Skimg = RandomCrop(256)(Cimg, Simg, Skimg)
        if random.random() < 0.5:
            Cimg, Simg, Skimg = Cimg.transpose(Image.FLIP_LEFT_RIGHT), Simg.transpose(Image.FLIP_LEFT_RIGHT), Skimg.transpose(Image.FLIP_LEFT_RIGHT)

        Cimg, Vimg, Simg, Skimg = self.transform(Cimg), self.vtransform(Cimg), self.stransform(Simg), self.stransform(Skimg)

        return Cimg, Vimg, Simg, Skimg
